...
class BalitaController extends VoyagerBaseController
{
    ...
    //
    //****************************************
    public function updateUmurColumn()
    {
    DB::table('balitas')
        ->update([
            'umur_tahun' => DB::raw('TIMESTAMPDIFF(YEAR, tgl_lahir, CURDATE())'),
            'umur_bulan' => DB::raw('TIMESTAMPDIFF(MONTH, tgl_lahir, CURDATE())'),
        ]);
        Session::flash('success', 'Umur berhasil diupdate.');
        return redirect()->back()->with('success', 'Umur berhasil diupdate.');
    }
